...
@dataclass
class SplitConfig:
    # Step1 输出目录（必须包含 images/ 和 labels/）
    src_dir: Path
    # Step2 输出目录（会生成 images/train,val 和 labels/train,val）
    out_dir: Path
    ...
    drop_excess: bool = True  # count模式下：如果总数 > train+val，是否丢弃多余样本

    seed: int = 42
    mode: str = "copy"  # "copy" or "move"

    # YOLO11-seg：你这里是 1 类：card
...
def main(cfg: SplitConfig) -> None:
    ...
    # count模式下可能出现 total > train+val
    if cfg.split_mode.lower().strip() == "count" and (n_train + n_val) < n_total:
        if cfg.drop_excess:
            selected = pairs[: (n_train + n_val)]
        else:
            # 不丢弃：剩余全部丢到 train（更常见）
            selected = pairs
