        ...
        try:
            if isinstance(item, (list, tuple)) and len(item) >= 2:
                ...
                if isinstance(tc, (list, tuple)) and len(tc) >= 2:
                    ...
                    # 1. 提高置信度门槛：倒置文字通常置信度较低，0.75 能过滤大部分乱码
                    if conf < 0.75:
                        continue
                    
                    # 2. 忽略单字：倒置时容易把局部纹理识别为单个符号，而名片有效信息通常>=2字
                    if len(text) < 2:
                        continue
...
def best_upright_rotation_4way(
    ...
    # [DEBUG] 打印四个方向的分数，方便排查为什么不回正
    print(f"    [Upright Scores] 0°:{scored[0][0]:.2f}  90°:{scored[1][0]:.2f}  180°:{scored[2][0]:.2f}  270°:{scored[3][0]:.2f}")
...
class PredictWarpConfig:
    ...
    ocr_lang: str = "japan"
    upright_min_abs_score: float = 2.0
    upright_min_ratio: float = 1.01
...
def parse_args() -> PredictWarpConfig:
    ...
    ap.add_argument("--upright_min_abs_score", type=float, default=2.0)
    ap.add_argument("--upright_min_ratio", type=float, default=1.01)
...
def default_cfg() -> PredictWarpConfig:
    ...
    return PredictWarpConfig(
        ...
        out_dir="outputs/step4_cards_ocr_v1",
        imgsz=1024,
        conf=0.25,
        iou=0.85,
        device="0",
        ...
        auto_upright=True,
        ocr_lang="japan",
        upright_min_abs_score=2.0,
        upright_min_ratio=1.01,
